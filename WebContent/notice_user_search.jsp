		...
		<div id="box">
   		<form id="user_admin" action="MemberController.do" method="post" >
   			...
   			<table class="table table-striped" id="box_tb" >
   				...
   				%>
   					<td><%=dto.getNt_no() %></td>
   					<td><a style="color:blue;" href="NoticeController.do?command=notice_user_detail&nt_no=<%=dto.getNt_no() %>"><%=dto.getNt_title() %></a></td>
   				...
   				%>
   				<tr>
   					<td colspan="6" align="right"><input class="btn btn-secondary mb-3" type="button" value="전체 목록으로" onclick="location.href='NoticeController.do?command=notice_list&nt_no=1'" /></td>
   				</tr>
   			</table>
		</form>
		</div>
