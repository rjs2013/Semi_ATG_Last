	...
	<div class="nav_cus sticky-top">
		...
		<!-- 개인정보 수정 전 비밀번호 확인  -->
			...
			<fieldset class="row text-center">
				<legend class="col-form-label col-sm-4 pt-0"><h4>비밀번호 확인</h4></legend>
				<form class="p-4 " action="MemberController.do" method="post" >
				<input type="hidden" name="command" value="member_pw_check" />
				<input type="hidden" name="mb_no" value="<%=LDto.getMb_no() %>" />
					<div class="form-group row">
						<div class="pl-3">
						<input type="password" class="form-control" id="pw_check" name="pw_check" placeholder="비밀번호를 입력하세요" required="required" /> 
						</div>
				
						<div class="pl-3">
						<input type="submit" class="btn btn-custom" value="확인" />
						<button class="btn btn-custom" onclick="location.href='main.jsp'">취소</button>
						</div>
					</div>
				</form>
			</fieldset>	
